...
class UserController {
  ...
  async getHistories(req, res) {
    ...
    if (sort) {
      ...
      let value = sort.split(":")[1];
      query.order = [
        [sequelize.literal("`comics_history`." + "`" + col + "`"), value],
      ];
    }
    query = {
    ...
    query.include = [
      {
        association: "comics_history",
        attributes: ["comic_id", "comic_name", "comic_img"],
        through: {
          attributes: ["chapters", "reading_chapter_name"],
        },
      },
    ];

    try {
      let countComics = await History.count({ where: { user_uuid: uuid } });
